  ...
  return (
    <>
      ...
      <aside id="mobile-navigation" ref={drawerRef} role="dialog" aria-modal="true" aria-label="Site navigation" className="fixed right-0 top-0 z-[100] flex h-dvh w-full flex-col border-l border-slate-200 bg-white shadow-2xl dark:border-slate-800 dark:bg-slate-950 sm:w-[min(92%,26rem)]">
        <header className="flex min-h-[4.25rem] items-center justify-between border-b border-slate-200 px-4 py-3 dark:border-slate-800 sm:px-5 sm:py-4">
          <Link href="/" onClick={onClose} className="flex items-center gap-3"><span className="flex h-9 w-9 items-center justify-center rounded-xl bg-slate-950 text-sm font-black text-white dark:bg-amber-400 dark:text-slate-950">C</span><span><strong className="block text-lg leading-none tracking-tight">Contextra</strong><span className="mt-1 block text-[0.58rem] font-bold uppercase tracking-[0.16em] text-slate-500 dark:text-slate-400">News with context</span></span></Link>
